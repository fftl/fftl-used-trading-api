...
class ReviewControllerTest {
    ...
    void setUp() {
        ...
        review = Review.builder()
            .id(1L)
            .content("상품 괜찮은 것 같아요. 좋은 거래 감사합니다!")
            .product(product)
            .user(user)
            .build();
    ...
    void saveReview() throws Exception{
        ...
        JSONObject jsonObject = new JSONObject()
            .put("content", "상품 괜찮은 것 같아요. 좋은 거래 감사합니다!")
            .put("userId", 1L)
            .put("productId",1L);
        ...
        actions
            ...
            .andDo(document("saveReview",
                    requestFields(
                        ...
                        fieldWithPath("userId").type(JsonFieldType.NUMBER).description("작성자 key"),
                        fieldWithPath("productId").type(JsonFieldType.NUMBER).description("후기 작성 상품 key")
                    ),
                    responseFields(
                        ...
                        fieldWithPath("data.id").type(JsonFieldType.NUMBER).description("등록된 후기 key"),
                        fieldWithPath("data.content").type(JsonFieldType.STRING).description("등록된 후기 내용"),
                        fieldWithPath("data.userId").type(JsonFieldType.NUMBER).description("후기 작성자 key"),
                        fieldWithPath("data.productId").type(JsonFieldType.NUMBER).description("후기 작성 상품 key")
                )));
    }

    @DisplayName("후기 수정하기 테스트")
    @Test
    void updateReview() throws Exception{
        ...
        actions
            ...
            .andDo(document("updateReview",
                    pathParameters(
                      parameterWithName("reviewId").description("후기 key")
                    ),
                    requestParameters(
                        parameterWithName("content").description("수정 후기 내용")
                    ),
                    responseFields(
                        fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("요청 성공여부"),
                        fieldWithPath("message").type(JsonFieldType.STRING).type(JsonFieldType.NULL).description("요청 실패시 메시지"),
    ...
    void deleteReview() throws Exception{
        ...
        actions
            ...
            .andDo(document("deleteReview",
                    pathParameters(
                      parameterWithName("reviewId").description("삭제할 후기 key")
                    ),
                    responseFields(
                        fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("요청 성공여부"),
                        fieldWithPath("message").type(JsonFieldType.STRING).type(JsonFieldType.NULL).description("요청 실패시 메시지"),
                        fieldWithPath("data").type(JsonFieldType.VARIES).description("요청 메시지")
                )));
